...
% 读进去其中一天的污染物数据，将四种污染物画在同一张图上。读取的1日2日的数据。
for k=30  %用于读取不同的文件夹
for j=k+1     %%！！！！！！改改改！！！！！！！！
    ncfile1=['F:\graduate\毕业论文\数据\othersdata\cmaq\201712',sprintf('%02d',k),'\METCRO2D.CN09GD_201712',sprintf('%02d',j)] %读不同文件下的不同文件%！！！！改改改！！！！！！！！
    ncfile2=['F:\graduate\毕业论文\数据\othersdata\cmaq\201712',sprintf('%02d',k),'\METDOT3D.CN09GD_201712',sprintf('%02d',j)]
for i=17:24 % 每个文件第一个时次是北京时09时，第24个时次是次日08时。
...
if(j==k+1)
    ...
    print(gcf,'-dpng','-r200',['F:\graduate\毕业论文\数据\picture\预报第1天\q201801',sprintf('%02d',j-31),str]); %%！！！！！！改改改！！！！！！！！
    else
    str=sprintf('%02d',i-16);
     suptitle(['201801',sprintf('%02d',j+1-31),'/',str])  %给分块矩阵加总标题
    print(gcf,'-dpng','-r200',['F:\graduate\毕业论文\数据\picture\预报第1天\q201801',sprintf('%02d',j+1-31),str]);%保存到不同的文件加下 %%！！！！！！改改改！！！！！！！！
    end
end
if(j==k+2)
   if(i<17)
