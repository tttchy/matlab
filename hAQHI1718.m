...
subplot(2,3,5);
temp1=ncread(ncfile,'PM25');
temp1=squeeze(temp1(:,:,1,i+4));
temp2=ncread(ncfile,'O3');
temp2=squeeze(temp2(:,:,1,i+4));
temp3=ncread(ncfile,'NO2');
temp3=squeeze(temp3(:,:,1,i+4));
temp4=ncread(ncfile,'SO2');
temp4=squeeze(temp4(:,:,1,i+4));
...
temp1=ncread(ncfile,'PM25');
temp1=squeeze(temp1(:,:,1,i+5));
temp2=ncread(ncfile,'O3');
temp2=squeeze(temp2(:,:,1,i+5));
temp3=ncread(ncfile,'NO2');
temp3=squeeze(temp3(:,:,1,i+5));
temp4=ncread(ncfile,'SO2');
temp4=squeeze(temp4(:,:,1,i+5));
...
if(j==k+1)
    ...
    print(gcf,'-dpng','-r200',['F:\graduate\毕业论文\数据\picture\预报第1天\AQHI201801',sprintf('%02d',j-31),str])%%！！！！！！改改改！！！！！！！！
    else
    str=sprintf('%02d',i-16);
     suptitle(['201801',sprintf('%02d',j+1-31)])  %给分块矩阵加总标题
    print(gcf,'-dpng','-r200',['F:\graduate\毕业论文\数据\picture\预报第1天\AQHI201801',sprintf('%02d',j+1-31),str])%保存到不同的文件加下 %%！！！！！！改改改！！！！！！！！
    end
end
