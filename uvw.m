...
%通过改变标量可以画风场图和水汽输送图。
m=7;
usum(1:19,1:m,1)=0;%%sum(46:64,1,1),矩阵的最大维数是sum(64,1,1).
vsum(1:19,1:m,1)=0;
wsum(1:19,1:m,1)=0;
...
usum(1:19,1:m,i+1)=U1+usum(1:19,1:m,i);
end
uwind2(1:19,1:m,j)=usum(1:19,1:m,17);%第j天的世界时前16个时刻，北京时的9-24时
...
uwind3(1:19,1:m,j)=usum(1:19,1:m,25);%第j天的世界时的后8个时刻，北京时的第j+1天的1-8时。

for i=1:16;
V1=ncread(ncfile2,'VWIND');
V1=squeeze(V1(46:64,44,1:m,i));
vsum(1:19,1:m,i+1)=V1+vsum(1:19,1:m,i);
end
vwind2(1:19,1:m,j)=vsum(1:19,1:m,17);%第j天的世界时前16个时刻，北京时的9-24时
vsum(1:19,1:m,17)=0;
for i=17:24;
V2=ncread(ncfile2,'VWIND');
V2=squeeze(V2(46:64,44,1:m,i));
vsum(1:19,1:m,i+1)=V2+vsum(1:19,1:m,i);
end
vwind3(1:19,1:m,j)=vsum(1:19,1:m,25);%第j天的世界时的后8个时刻，北京时的第j+1天的1-8时。

uwind1(1:19,1:m,j)=(uwind2(1:19,1:m,j)+uwind3(1:19,1:m,j-1));  %%%AQ英语计算北京时的日平均，
vwind1(1:19,1:m,j)=(vwind2(1:19,1:m,j)+vwind3(1:19,1:m,j-1));  %%%AQ英语计算北京时的日平均，
wind2(1:19,1:m,j)=uwind1(1:19,1:m,j).*uwind1(1:19,1:m,j);
wind3(1:19,1:m,j)=vwind1(1:19,1:m,j).*vwind1(1:19,1:m,j);
wind1(1:19,1:m,j)=(sqrt(wind2(1:19,1:m,j)+wind3(1:19,1:m,j)))/24;  %%%AQ英语计算北京时的日平均，

ws=wind1(1:19,1:m,j);
uw=uwind1(1:19,1:m,j)/24;
vw=vwind1(1:19,1:m,j)/24
...
hold on;
quiver(xx1,zz1,uw,vw,0.5,'color','k');%以矢量箭头的方式画图，10是调节箭头长度.
hold off;
...
m_grid; 
title(['UVlon201801',sprintf('%02d',j)]);
...
print(gcf,'-dpng','-r200',['F:\graduate\毕业论文\数据\picture\预报第1天\DAVEuvw-201801',sprintf('%02d',j)]);%输出到文件夹下。
