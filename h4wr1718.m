...
% 读进去其中一天的污染物数据 将四种污染物的浓度画在一张图上，画1718穿插的
for k=31 %用于读取不同的文件夹
for j=k+1     %%！！！！！！改改改！！！！！！！！
    ncfile=['F:\graduate\毕业论文\数据\othersdata\cmaq\201712',sprintf('%02d',k),'\CONC.CN09GD_201801',sprintf('%02d',j-31)] %读不同文件下的不同文件%！！！！改改改！！！！！！！！
    %ncfile2=['F:\graduate\毕业论文\数据\othersdata\cmaq\201712',sprintf('%02d',k),'\METDOT3D.CN09GD_201801',sprintf('%02d',j-31)]
...
if(j==k+1)
    ...
    print(gcf,'-dpng','-r200',['F:\graduate\毕业论文\数据\picture\预报第1天\p201801',sprintf('%02d',j-31),str]); %%！！！！！！改改改！！！！！！！！
    else
    str=sprintf('%02d',i-16);
     suptitle(['201801',sprintf('%02d',j+1-31),'/',str])  %给分块矩阵加总标题
    print(gcf,'-dpng','-r200',['F:\graduate\毕业论文\数据\picture\预报第1天\p201801',sprintf('%02d',j+1-31),str]);%保存到不同的文件加下 %%！！！！！！改改改！！！！！！！！
    end
end
if(j==k+2)
   if(i<17)
