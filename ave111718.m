...
A=zeros(24,1);
for k=30%用于读取不同的文件夹
for j=31    %读取上一文件夹下的文件
    ncfile=['F:\graduate\毕业论文\数据\othersdata\cmaq\201712',sprintf('%02d',k),'\CONC.CN09GD_201712',sprintf('%02d',j)] %读不同文件下的不同文件%！！！！改改改！！！！！！！！
...
A
%xlswrite('F:\graduate\毕业论文\数据\picture\avetime1.xls',A,'sheet1','A:C')%'A:C'表示保存在A和C列单元格中，并占满。
%filename=['F:\graduate\毕业论文\数据\picture\201801',sprintf('%02d',k),'\avetime1.xls']
%xlswrite(filename,A,'sheet1','M:O')%保存到exce文件中去
end
