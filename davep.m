clear all;

ncfilelocat1='F:\graduate\毕业论文\数据\GRIDCRO2D_CN09GD';
ncfilelocat2='F:\graduate\毕业论文\数据\GRIDDOT2D_CN09GD';
lonmodel1=ncread(ncfilelocat1,'LON');%读取二维气象要素的经度数据。
latmodel1=ncread(ncfilelocat1,'LAT');
lonmodel2=ncread(ncfilelocat2,'LOND');%读取三维风场的经度数据
latmodel2=ncread(ncfilelocat2,'LATD');
% 读进去其中一天的污染物数据
%画气象要素的标量
psum(102,81,1,25)=0;
vsum(102,81,1,25)=0;
rsum(102,81,1,25)=0;
for k=1:25 %用于读取不同的文件夹
   % subplot(3,4,k-12); 分块
for j=k+1 ;   %%！！！！！！改改改！！！！！！！！
    ncfile=['I:\cmaqout\tianchunyan\201801',sprintf('%02d',k),'\METCRO2D.CN09GD_201801',sprintf('%02d',j)]; %读不同文件下的不同文件%！！！！改改改！！！！！！！！
    ncfile2=['I:\cmaqout\tianchunyan\201801',sprintf('%02d',k),'\METDOT3D.CN09GD_201801',sprintf('%02d',j)];
%%%%%%%%%%%%%%%%%%%%画气压场

for i=1:16;
T1=ncread(ncfile,'RN');
T1(:,:,1,i)=squeeze(T1(:,:,1,i));
rsum(:,:,1,i+1)=T1(:,:,1,i)+rsum(:,:,1,i);
end
wind2(:,:,1,j)=rsum(:,:,1,17);%第j天的世界时前16个时刻，北京时的9-24时

rsum(:,:,1,17)=0 ;
for i=17:24;
T2=ncread(ncfile,'RN');
T2(:,:,1,i)=squeeze(T2(:,:,1,i));
rsum(:,:,1,i+1)=T2(:,:,1,i)+rsum(:,:,1,i);
end
wind3(:,:,1,j)=rsum(:,:,1,25);%第j天的世界时的后8个时刻，北京时的第j+1天的1-8时。

wind1(:,:,1,j)=(wind2(:,:,1,j)+wind3(:,:,1,j-1))/24;  %%%AQ英语计算北京时的日平均。
%%%%%%%%%%%%%%%%%%%

load D:\matlab\MATLAB2014b\MPL_Blues.mat;
m_proj('mercator','lon',[112.85,114.2],'lat',[22.37,24.1]);  %创建一个自己设定经纬度范围的绘图投影区域
m_contourf(lonmodel1,latmodel1,wind1(:,:,1,j)*10*24,32,'linestyle','none');   %使用填色图的形式画dispvar这个变量 'LevelList',[10]  'linestyle','none'
hold on;
m_contourf(lonmodel1,latmodel1,wind1(:,:,1,j)*10*24,'LevelList',[10],'color','r');   %使用填色图的形式画dispvar这个变量 'LevelList',[10]  'linestyle','none'
hold off;
%[c,h]=m_contour(lonmodel1,latmodel1,wind1(:,:,1,j)*10*24,'LevelList',[10]);%画指定等值线
%clabel(c,h);
%grid,xlabel('x-axis'),ylabel('y-axis')
%set(gca,'FontSize',5);%只能同时改变x y轴显示的字体大小；
%m_contour(lonmodel1,latmodel1,wind1(:,:,1,j)/100,15,'color','r');%使用等值线的形式画dispvar这个变量,10表示所画的等值线的数目。
m_plotbndry('D:\matlab\MATLAB2014b\m_map_chn\m_map\diqujie','color','k','linewidth',1.5);  %绘制省界
m_plotbndry('D:\matlab\MATLAB2014b\m_map_chn\m_map\xianjie','color','k','linewidth',0.3); %绘制市界
m_grid; 
title(['RAIN-201801',sprintf('%02d',j)]);  %绘图上的标题
colormap(MPL_Blues);
hc=colorbar; %色标
caxis([0 12]);%设定色标的颜色对应值得范围为0-11之间。大于11的将射到11上，与实际值对应。
%set(gca,'clim',[980 1025]);  %设定绘图要素的取值范围
set(get(hc,'ylabel'),'String','MM','fontsize',10);  % 对色标加单位
print(gcf,'-dpng','-r200',['F:\graduate\毕业论文\数据\picture\预报第1天\RAIN-201801',sprintf('%02d',j)]);%输出到文件夹下。
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

end
end
%hc=colorbar('position',[0.909 0.2 0.02 0.6]) 
%set(get(hc,'ylabel'),'String','hPa','fontsize',10);  % 对色标加单位
%print(gcf,'-dpng','-r200',['F:\graduate\毕业论文\数据\picture\预报第1天\DAVEPRSFC-201801-2']);%输出到文件夹下。
